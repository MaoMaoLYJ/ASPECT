...
from verl.utils.vllm import VLLM_LORA_PATH
from verl.workers.config import HFModelConfig, RewardModelConfig, RolloutConfig
from verl.workers.rollout.lora_route_contract import require_explicit_lora_route_loaded
...
class vLLMHttpServerBase:
    ...
    def __init__(
        ...
        self._audited_explicit_lora_routes: set[int] = set()
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None):
        ...
        if self.model_config.lora_rank > 0:
            ...
            if self.replica_rank == 0:
                logger.info(f"[Multi-Agent LoRA] Configured vLLM with max_loras={max_loras}")
    ...
    async def generate(
        ...
        full_parameter_route: Optional[str] = None,
    ) -> TokenOutput:
        """Generate sequence with token-in-token-out."""
        routes = list(getattr(self.model_config, "full_parameter_routes", []))
        if routes:
            if full_parameter_route not in routes or lora_int_id is not None:
                raise ValueError("Explicit independent full-parameter route required; no LoRA or default fallback")
            if not hasattr(self, "full_parameter_batcher"):
                from verl.experimental.agent_loop.full_parameter_batcher import FullParameterBatcher
                self.full_parameter_batcher = FullParameterBatcher(self._activate_full_parameter_route)
            return await self.full_parameter_batcher.submit(
                full_parameter_route,
                lambda _: self._generate_tokens(prompt_ids, sampling_params, request_id, image_data, None),
            )
        if full_parameter_route is not None:
            raise ValueError("Full-parameter route passed to a shared server")
        return await self._generate_tokens(prompt_ids, sampling_params, request_id, image_data, lora_int_id)

    async def _activate_full_parameter_route(self, route):
        # The per-engine batcher has drained every request before entering here.
        await self.engine.reset_prefix_cache()
        result = await self.engine.collective_rpc("activate_full_parameter_route", args=(route,))
        if result != [route]:
            raise RuntimeError(f"Full-parameter TP1 route activation mismatch: {result}")

    async def _generate_tokens(self, prompt_ids, sampling_params, request_id, image_data, lora_int_id):
        ...
        max_tokens = (
            ...
            else min(available_tokens, int(requested_max_tokens))
        )
        if max_tokens <= 0:
            raise RuntimeError(
                f"No generation capacity remains: prompt={len(prompt_ids)} "
                f"model_limit={self.config.max_model_len}"
            )
        sampling_params["logprobs"] = 0 if sampling_params.pop("logprobs", False) else None
        sampling_params.setdefault("repetition_penalty", self.config.get("repetition_penalty", 1.0))
        # This token-in/token-out API returns only the final result. Avoid
        # constructing and delivering unused cumulative outputs at every token.
        sampling_params["output_kind"] = RequestOutputKind.FINAL_ONLY
        # The caller decodes token IDs. Retain backend decoding when text stop
        # strings need it, or when the caller explicitly requests it.
        ...
        # Add lora request
        lora_request = None
        explicit_lora_int_id = lora_int_id
        if explicit_lora_int_id is not None and self.model_config.lora_rank <= 0:
            require_explicit_lora_route_loaded(
                explicit_lora_int_id, (), model_lora_rank=self.model_config.lora_rank
            )
        if self.model_config.lora_rank > 0:
            ...
            require_explicit_lora_route_loaded(
                explicit_lora_int_id,
                list_loras,
                model_lora_rank=self.model_config.lora_rank,
            )
            if (
                explicit_lora_int_id is not None
                and explicit_lora_int_id not in self._audited_explicit_lora_routes
            ):
                self._audited_explicit_lora_routes.add(explicit_lora_int_id)
                logger.info(
                    "[LoRARouteLoadAudit] requested=%s loaded=%s rank=%s",
                    explicit_lora_int_id,
                    sorted(list_loras),
