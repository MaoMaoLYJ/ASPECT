...
from verl.utils.vllm import VLLM_LORA_PATH, TensorLoRARequest, VLLMHijack, is_version_ge
...
class vLLMRollout(BaseRollout):
    def __init__(
        ...
        self.lora_kwargs = (
            {"enable_lora": True, "max_loras": config.max_loras, "max_lora_rank": get_vllm_max_lora_rank(lora_rank)}
...
class vLLMAsyncRollout(BaseRollout):
    ...
    async def _execute_method(self, method: str | bytes, *args, **kwargs):
        ...
        elif method == "activate_full_parameter_route":
            route = args[0]
            if route not in self.model_config.full_parameter_routes:
                raise ValueError(f"Unknown full-parameter rollout route: {route}")
            self.full_parameter_rollout_bank.activate(route)
            return route
    ...
    async def update_weights(
        ...
        **kwargs,
        ...
        if peft_config and base_sync_done:
            ...
            patch_vllm_moe_model_weight_loader(model)
            model.load_weights(weights)
            route = kwargs.get("full_parameter_route")
            if route is not None:
                from verl.workers.actor.full_parameter_bank import TensorBank
                if route not in self.model_config.full_parameter_routes:
                    raise ValueError(f"Unexpected full-parameter sync route: {route}")
                if not hasattr(self, "full_parameter_rollout_bank"):
                    self.full_parameter_rollout_bank = TensorBank(model)
                self.full_parameter_rollout_bank.capture(route)
