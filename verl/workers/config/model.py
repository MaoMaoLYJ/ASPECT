...
class HFModelConfig(BaseConfig):
    ...
    lora_rank: int = 0
    require_full_parameter_training: bool = False
    full_parameter_routes: list[str] = field(default_factory=list)
    lora_alpha: int = 16
    ...
    # Agent-wise LoRA-XS basis, route, and core initialization settings.
    agent_loraxs: dict = field(default_factory=dict)

    # Agent-wise LoRA-SB update-aligned basis and route settings.
    agent_lorasb: dict = field(default_factory=dict)
    peft_weight_warm_start: dict = field(default_factory=dict)
