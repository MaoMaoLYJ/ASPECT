...
class RolloutConfig(BaseConfig):
    ...
    replica_sticky_max_load_gap: int = -1
    ...
    # Group agent-loop requests by LoRA adapter before admitting them to the
    # inference server. These settings affect scheduling only; each request
    # retains its own prompt, sampling parameters, request ID, and adapter ID.
    route_homogeneous_batching: bool = False
    route_homogeneous_batch_size: int = 256
    route_homogeneous_max_requests_per_turn: int = 512
    route_homogeneous_coalesce_ms: float = 2.0
    route_semantic_canary: bool = False
    route_semantic_canary_max_tokens: int = 16
    route_semantic_canary_logprob_atol: float = 1e-5
    route_diagnostics_log_interval: int = 256
    require_explicit_lora_route: bool = False
