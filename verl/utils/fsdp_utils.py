...
def collect_multi_adapter_lora_params(module: FSDP, adapter_names: list[str], layered_summon: bool) -> OrderedDict:
    """Export ordinary PEFT adapters from one read-only FSDP1 snapshot.

    Gathering the whole model once per adapter repeats base-weight collectives.
    Keep PEFT's adapter filtering, CPU transfer and route order unchanged, and
    leave layered/FSDP2 collection on its existing memory-management path.
    """
    from peft.utils.save_and_load import get_peft_model_state_dict

    adapter_names = list(adapter_names)
    if not adapter_names or len(set(adapter_names)) != len(adapter_names):
        raise ValueError("Expected a non-empty list of unique LoRA adapter names")
    peft_model = getattr(module, "_fsdp_wrapped_module", module)
    missing = set(adapter_names) - set(peft_model.peft_config)
    if missing:
        raise ValueError(f"LoRA adapters missing from peft_config: {sorted(missing)}")

    if fsdp_version(module) != 1 or layered_summon:
        return OrderedDict(
            (name, collect_lora_params(module=module, layered_summon=layered_summon,
                                       base_sync_done=True, adapter_name=name))
    ...
    with FSDP.summon_full_params(module, writeback=False):
        ...
        for name in adapter_names:
            params = get_peft_model_state_dict(peft_model, state_dict=state_dict, adapter_name=name)
            adapters[name] = {
                key: param.full_tensor().detach().cpu()
                if hasattr(param, "full_tensor")
                else param.detach().cpu()
                for key, param in params.items()
            }
        # Do not keep unsharded GPU views alive after leaving the gather context.
