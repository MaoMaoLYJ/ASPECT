...
# TensorLoRARequest carries the adapter in memory, so this path is only an API
# sentinel. It must be absolute: vLLM treats a missing relative path as a
# Hugging Face repository and otherwise attempts a network download.
VLLM_LORA_PATH = "/tmp/verl_tensor_lora_adapter"
