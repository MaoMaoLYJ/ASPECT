...
class DeepcodeOrchestratorWorkersWorkflow(OrchestratorWorkersWorkflow):
    ...
    async def run(self, task: Dict[str, Any], uid: str, **kwargs):
        episode = await super().run(task, uid, **kwargs)
        self._add_paper_diagnostics(episode)
        return episode

    def _add_paper_diagnostics(self, episode) -> None:
        workers = [
            trajectory
        ...
        worker_outputs = [trajectory.steps[-1].model_output for trajectory in workers]
        worker_responses = [str(trajectory.steps[-1].action) for trajectory in workers]
        episode.metrics.update(
            code_response_metrics(worker_outputs, prefix="paper_diag/worker")
        )
        if worker_responses:
            episode.metrics["paper_diag/worker/pairwise_3gram_jaccard"] = (
                mean_pairwise_ngram_jaccard(worker_responses, n=3)
            )
        if orchestrators:
            episode.metrics.update(
                text_response_metrics(
                    [orchestrators[-1].steps[-1].model_output],
                    prefix="paper_diag/orchestrator",
                )
            )
            action = orchestrators[-1].steps[-1].action
            strategies = action.get("strategies", []) if isinstance(action, dict) else []
            episode.metrics["paper_diag/orchestrator/strategy_count"] = float(
                len(strategies)
            )
            episode.metrics["paper_diag/orchestrator/strategy_3gram_jaccard"] = (
                mean_pairwise_ngram_jaccard(strategies, n=3)
            )
        if synthesizers:
            episode.metrics.update(
                code_response_metrics(
                    [synthesizers[-1].steps[-1].model_output],
                    prefix="paper_diag/synthesizer",
                )
            )
