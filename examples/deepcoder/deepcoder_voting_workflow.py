...
from rllm.workflows.code_test_loop_mixin import CodeTestLoopMixin
...
class DeepcodeVotingWorkflow(CodeTestLoopMixin, VotingWorkflow):
    ...
    async def _generate_single_with_feedback(
        ...
        route_name = (
            f"{self.GENERATOR_NAME}{vote_index}"
            if self.agent_loraxs_instance_routing
            else self.GENERATOR_NAME
        )
        rollout_kwargs = {"agent_name": route_name}
        if uid is not None:
            rollout_kwargs["application_id"] = uid
        output = await self.rollout_engine.get_model_response(messages, **rollout_kwargs)

        response = self.extract_response(output)

        trajectory = Trajectory(
            name=f"{self.GENERATOR_NAME}{vote_index}",
            steps=[
                Step(
    ...
    async def run(self, task: Dict[str, Any], uid: str, **kwargs) -> Episode:
        ...
        if self.enable_test_loop:
            episode = await self._run_with_test_loop(task, uid)
        else:
            episode = await self._run_single_pass(task, uid)
        self._add_paper_diagnostics(episode)
        return episode

    def _add_paper_diagnostics(self, episode: Episode) -> None:
        generator_trajectories = [
            trajectory
            for trajectory in episode.trajectories
            if trajectory.name.startswith(self.GENERATOR_NAME)
        ]
        aggregator_trajectories = [
            trajectory
            for trajectory in episode.trajectories
            if trajectory.name == self.AGGREGATOR_NAME
        ]
        generator_outputs = [
            trajectory.steps[-1].model_output for trajectory in generator_trajectories
        ]
        generator_responses = [
            str(trajectory.steps[-1].action) for trajectory in generator_trajectories
        ]
        aggregator_outputs = [
            trajectory.steps[-1].model_output for trajectory in aggregator_trajectories
        ]
        episode.metrics.update(
            code_response_metrics(
                generator_outputs,
                prefix="paper_diag/generator",
            )
        )
        episode.metrics.update(
            aggregator_response_metrics(
                aggregator_outputs,
                prefix="paper_diag/aggregator",
            )
        )
        if generator_responses:
            episode.metrics["paper_diag/generator/pairwise_3gram_jaccard"] = (
                mean_pairwise_ngram_jaccard(generator_responses, n=3)
            )
        if aggregator_trajectories:
            selected = str(aggregator_trajectories[-1].steps[-1].action)
            episode.metrics.update(
                code_response_metrics(
                    [SimpleNamespace(content=selected, token_ids=None)],
                    prefix="paper_diag/selected",
                )
            )
    ...
    async def _run_single_pass(
        ...
        generation_tasks = [
            self._generate_single_with_feedback(task, i, None, uid=uid)
            for i in range(self.n_votes)
        ]
        generator_trajectories = await asyncio.gather(*generation_tasks)

        # Extract responses
        responses = [traj.steps[0].action for traj in generator_trajectories]
        ...
        # Only evaluate the final selected response - skip individual generator rewards
        # since use_final_outcome_reward propagates the final reward to all trajectories
    ...
    async def _run_with_test_loop(
        ...
        for test_round in range(self.max_test_rounds):
            # Step 1: Generate N responses in parallel
            generation_tasks = [
                self._generate_single_with_feedback(
                    task, i, test_feedback, uid=uid
                )
                for i in range(self.n_votes)
            ]
            generator_trajectories = await asyncio.gather(*generation_tasks)
        ...
        if generator_trajs:
            ...
            for traj, gen_reward in zip(generator_trajs, gen_rewards, strict=True):
