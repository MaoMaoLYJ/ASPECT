...
class DeepcodeEvaluatorOptimizerWorkflow(CodeTestLoopMixin, EvaluatorOptimizerWorkflow):
    ...
    def _add_paper_diagnostics(self, episode: Episode) -> None:
        ...
        evaluator_outputs = [
            trajectory.steps[-1].model_output
            for trajectory in episode.trajectories
            if trajectory.name.startswith(self.EVALUATOR_NAME)
        ]
        ...
        episode.metrics.update(
            evaluator_response_metrics(
                evaluator_outputs,
                prefix=f"paper_diag/{self.EVALUATOR_NAME}",
            )
        )
        if generator_outputs:
            episode.metrics.update(
                code_response_metrics(
                    generator_outputs[:1],
                    prefix=f"paper_diag/{self.GENERATOR_NAME}_iter1",
                )
            )
        if len(generator_outputs) > 1:
            episode.metrics.update(
                code_response_metrics(
                    generator_outputs[1:],
                    prefix=f"paper_diag/{self.GENERATOR_NAME}_refinement",
                )
            )
        if evaluator_outputs:
            episode.metrics.update(
                evaluator_response_metrics(
                    evaluator_outputs[:1],
                    prefix=f"paper_diag/{self.EVALUATOR_NAME}_iter1",
                )
            )
