...
class OrchestratorWorkersWorkflow(Workflow):
    ...
    def __init__(
        ...
        agent_loraxs_instance_routing: bool = False,
        agent_lorasb_instance_routing: bool = False,
        agent_wise_lora_instance_routing: bool = False,
        agent_full_parameter_instance_routing: bool = False,
        **kwargs,
    ):
        ...
        self.agent_loraxs_instance_routing = bool(
            agent_loraxs_instance_routing or agent_lorasb_instance_routing or agent_wise_lora_instance_routing
            or agent_full_parameter_instance_routing
        )
    ...
    async def run(self, task: Dict[str, Any], uid: str, **kwargs) -> Episode:
        ...
        if len(proposal.strategies) > self.max_subtasks:
            ...
            failure_metrics.update(
                text_response_metrics(
                    [proposal_output],
                    prefix=f"paper_diag/{self.ORCHESTRATOR_NAME}",
                )
            )
            failure_metrics[
                f"paper_diag/{self.ORCHESTRATOR_NAME}/strategy_3gram_jaccard"
            ] = mean_pairwise_ngram_jaccard(proposal.strategies, n=3)
            failure_metrics[
                f"paper_diag/{self.ORCHESTRATOR_NAME}/first_strategy_label"
            ] = normalized_first_strategy_label(proposal.strategies)
            return Episode(
                id=uid,
                task=task,
    ...
    def _compute_workflow_metrics(
        ...
        orchestrator_outputs = [
            trajectory.steps[-1].model_output
            for trajectory in trajectories
            if trajectory.name == self.ORCHESTRATOR_NAME
        ]
        worker_trajectories = [
            trajectory
            for trajectory in trajectories
            if trajectory.name.startswith(self.WORKER_NAME)
        ]
        worker_outputs = [
            trajectory.steps[-1].model_output for trajectory in worker_trajectories
        ]
        worker_responses = [
            str(trajectory.steps[-1].action) for trajectory in worker_trajectories
        ]
        synthesizer_outputs = [
            trajectory.steps[-1].model_output
            for trajectory in trajectories
            if trajectory.name == self.SYNTHESIZER_NAME
        ]
        metrics.update(
            text_response_metrics(
                orchestrator_outputs,
        ...
        metrics.update(
            text_response_metrics(
                worker_outputs,
                prefix=f"paper_diag/{self.WORKER_NAME}",
            )
        )
        metrics.update(
            text_response_metrics(
                synthesizer_outputs,
                prefix=f"paper_diag/{self.SYNTHESIZER_NAME}",
            )
        )
        metrics[f"paper_diag/{self.WORKER_NAME}/pairwise_3gram_jaccard"] = (
            mean_pairwise_ngram_jaccard(worker_responses, n=3)
        )
        metrics[
            f"paper_diag/{self.ORCHESTRATOR_NAME}/strategy_3gram_jaccard"
        ] = mean_pairwise_ngram_jaccard(proposal.strategies, n=3)
