...
class VotingWorkflow(Workflow):
    ...
    async def run(self, task: Dict[str, Any], uid: str, **kwargs) -> Episode:
        ...
        aggregator_outputs = [aggregator_trajectory.steps[-1].model_output]
        metrics.update(
            text_response_metrics(
                generator_outputs,
                prefix=f"paper_diag/{self.GENERATOR_NAME}",
            )
        )
        metrics.update(
            aggregator_response_metrics(
                aggregator_outputs,
                prefix=f"paper_diag/{self.AGGREGATOR_NAME}",
            )
        )
        metrics[f"paper_diag/{self.GENERATOR_NAME}/pairwise_3gram_jaccard"] = (
            mean_pairwise_ngram_jaccard(responses, n=3)
        )
        metrics["paper_diag/voting/oracle_available"] = float(any_correct)
        metrics["paper_diag/voting/selected_correct"] = float(final_is_correct)
        metrics["paper_diag/voting/selection_error_with_oracle"] = float(
            any_correct and not final_is_correct
        )
