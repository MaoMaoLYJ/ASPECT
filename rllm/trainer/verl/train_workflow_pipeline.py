...
class PipelineTaskRunner:
    ...
    def run(self, config, workflow_class=None, workflow_args=None):
        ...
        if config.trainer.get("agent_wise_lora", False):
            if config.trainer.get("share_policy", False) or any(
                config.trainer.get(mode, {}).get("enable", False)
                or config.actor_rollout_ref.model.get(mode, {}).get("enable", False)
                for mode in ("agent_loraxs", "agent_lorasb")
            ):
                raise ValueError("AW-LoRA requires unshared ordinary LoRA, not a private-core method")
            names = list(config.trainer.get("agent_names", []))
            if not names or len(names) != len(set(names)):
                raise ValueError("AW-LoRA requires unique Agent instance routes")
