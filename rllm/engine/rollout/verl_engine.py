...
class VerlEngine(RolloutEngine):
    def __init__(self, config, rollout_manager, tokenizer, processor=None, **kwargs):
        ...
        self._audited_agent_routes: set[tuple[str, int]] = set()
    ...
    async def get_model_response(self, messages: list[dict], **kwargs) -> ModelOutput:
        ...
        full_routes = (list(self.config.actor_rollout_ref.model.full_parameter_routes)
                       if (self.config.trainer.get("agent_wise_full_parameter", False)
                           or self.config.trainer.get("role_wise_full_parameter", False)) else [])
        if full_routes:
            if agent_name not in full_routes:
                raise ValueError(f"Unknown full-parameter agent: {agent_name}")
            lora_int_id = None
        elif self.config.trainer.get("share_policy"):
            lora_int_id = None
        else:
            lora_int_id = self.config.trainer.get("agent_names").index(agent_name) + VLLM_LORA_INT_ID if agent_name is not None else None
            if agent_name is not None and (agent_name, lora_int_id) not in self._audited_agent_routes:
                self._audited_agent_routes.add((agent_name, lora_int_id))
                print(
                    "[AgentRouteMapAudit] "
                    f"agent={agent_name} lora_int_id={lora_int_id} "
                    f"agent_index={lora_int_id - VLLM_LORA_INT_ID}",
                    flush=True,
                )
        ...
        # Enforce the response budget in the backend, not only after generation.
        ...
        if full_routes:
            server = self.server_manager._choose_server(application_id)
            try:
                # Parallel workers share a sticky session, not a backend generation ID.
                token_output = await server.generate.remote(
                    prompt_ids=request_prompt_ids, image_data=image_data, sampling_params=sampling_params,
                    request_id=f"{application_id}_{uuid.uuid4().hex}", full_parameter_route=agent_name,
                )
            finally:
                self.server_manager._release_server(server)
        else:
