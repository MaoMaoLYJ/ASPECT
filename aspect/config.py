...
def build_overrides(*, method, workflow, task, scale, model, output, ray_temp,
                    ...
                    smoke=False, gpu_memory=0.85, tail_control=None, quick_smoke=False):
    if quick_smoke and not smoke:
        raise ValueError('The quick profile requires explicit --smoke and cannot be canonical')
    ...
    if full and (task != "math" or scale != "1.7b" or workflow == "voting"):
        raise ValueError("The validated Full-FT contract supports Qwen3-1.7B Math Eval-Opt and Orch-Workers")
    if (not smoke and (steps != 200 or gpus != 4)) or not 1 <= gpus <= 8 or steps < 1:
        raise ValueError("Formal cells use 200 updates on four GPUs; use --smoke for smaller non-canonical checks")
    ...
    if full:
        ...
        cfg.update({"actor_rollout_ref.model.lora_adapter_path": None,
                    ...
                    "trainer.save_freq": -1, "trainer.test_freq": 1 if smoke else 10,
                    "data.val_batch_size": 512, "+rllm.load_validation_dataset": True,
                    "+trainer.inline_full_validation": {
                        "enable": True, "expected_rows": 1412, "canonical": not smoke,
                        "workflow": workflow, "output": str(Path(output) / "validation"),
                        "dataset_sha256": None}})
        if tail_control:
            cfg["+trainer.inline_full_validation.tail_control_dir"] = str(tail_control)
    ...
    if smoke:
        cfg["trainer.gradient_diagnostics.interval"] = 1
        cfg["trainer.save_freq"] = -1 if full else 1
        cfg["trainer.paper_metrics.validation_checkpoint_interval"] = 1
        if not full and method != "ASPECT":
            cfg["+trainer.peft_eval_checkpoint"] = {"enable": True, "retain_interval": 1}
        cfg["trainer.experiment_name"] += "-SMOKE_NOT_CANONICAL"
    if quick_smoke:
        cfg['data.train_batch_size'] = 8
        cfg['actor_rollout_ref.actor.ppo_mini_batch_size'] = 8
        cfg['actor_rollout_ref.rollout.n'] = 4
        if full:
            cfg['data.val_batch_size'] = 8
            cfg['+trainer.inline_full_validation']['expected_rows'] = 8
    return cfg
