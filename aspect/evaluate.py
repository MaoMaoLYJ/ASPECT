"""Evaluate complete official splits using real adapter checkpoints."""
import argparse
import json
import os
import re
...
def build_command(run, output, temp, *, steps=None, gpus=4,
                  gpu_memory=0.85, port=8000):
    ...
    if not run['canonical']:
        raise ValueError('Only completed paper-reproduction runs can be evaluated by this entry point')
    if gpus < 1 or not 0 < gpu_memory < 1 or not 1024 <= port <= 65536 - gpus:
        raise ValueError('Invalid evaluation GPU, memory, or port budget')
    cfg = run['overrides']
    interval = cfg['trainer.save_freq']
    allowed = range(interval, cfg['trainer.total_training_steps'] + 1, interval)
    steps = steps if steps is not None else list(allowed)
...
def main():
    ...
    if ((root / 'INCOMPLETE.json').exists() or any(root.glob('INVALID_DO_NOT_USE*'))):
    ...
    command = build_command(run, output, temp, steps=args.steps,
                            gpus=args.gpus,
